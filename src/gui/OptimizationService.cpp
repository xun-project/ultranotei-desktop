...
OptimizationService::OptimizationService(QObject* parent)
    : QObject(parent),
      m_checkTimerId(-1),
...
void OptimizationService::walletOpened()
{
    Q_ASSERT(m_checkTimerId == -1);
    m_checkTimerId = startTimer(CHECK_TIMER_INTERVAL);
}

void OptimizationService::walletClosed()
{
    m_isSynchronized = false;
    if (m_checkTimerId != -1)
    {
...
void OptimizationService::synchronizationProgressUpdated()
{
    m_isSynchronized = false;
}

void OptimizationService::synchronizationCompleted()
{
    m_isSynchronized = true;
}

void OptimizationService::timerEvent(QTimerEvent *_event)
{
...
void OptimizationService::checkOptimization()
{
...
void OptimizationService::optimize()
{
...
void OptimizationService::delay()
{
    QThread::sleep(2);
}

void OptimizationService::ensureStarted()
{
    if (m_optimizationTimerId != -1)
    {
...
void OptimizationService::ensureStopped()
{
    if (m_optimizationTimerId == -1)
    {
        return;
